// lib/auth/organizer_auth/organiser_dashboard_page.dart
          ...
          return ListView.separated(
            itemBuilder: (_, i) {
              ...
              return ListTile(
                title: Text(e.title),
                subtitle: Text(
                  '${df.format(e.start)} → ${df.format(e.end)} • ${e.location}\n'
                  '${e.approved ? 'Approved' : 'Pending'} • '
                  '${e.isGlobal ? 'Global' : 'Private'} • '
                  'Org: ${e.orgId.isEmpty ? 'N/A' : e.orgId}',
                ),
                isThreeLine: true,
                trailing: PopupMenuButton<String>(
                  onSelected: (v) async {
                    if (v == 'edit') {
                      await Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (_) => CreateEditEventPage(user: user, existing: e),
                        ),
                      );
                    } else if (v == 'delete') {
