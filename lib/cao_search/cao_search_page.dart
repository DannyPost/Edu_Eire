...
class CourseProvider with ChangeNotifier {
  ...
  int _loadedCount = 20;
  String _search = '';
  String? _universityFilter, _locationFilter, _levelFilter, _jobFieldFilter;

  // Load user interests from Firestore and courses from CSV
  Future<void> loadUserInterestsAndCourses(String csvPath) async {
    await _loadUserInterests();
    await _loadCoursesFromCsv(csvPath);
  ...
  Future<void> _loadCoursesFromCsv(String csvPath) async {
    ...
    if (csvTable.length <= 1) return;

    _allCourses = csvTable.skip(1).map((row) => Course(
      row[0].toString().trim().split('.').first,
      row[1].toString().trim(),
      row[2].toString().trim(),
      row[3].toString().trim(),
      row[4].toString().trim(),
  ...
  void _applyPersonalization() {
    ...
    _allCourses.sort((a, b) {
      int scoreA = _relevanceScore(a);
      int scoreB = _relevanceScore(b);
      return scoreB.compareTo(scoreA); // highest score first
    });
  }

  int _relevanceScore(Course course) {
    int score = 0;
    for (var interest in _userInterests) {
      final interestLower = interest.toLowerCase();
      if (course.title.toLowerCase().contains(interestLower)) score += 3;
      if (course.jobField.toLowerCase().contains(interestLower)) score += 5;
    }
    return score;
  }
  ...
  void applyFilters() {
    _filtered = _allCourses.where((course) {
      final matchesSearch = course.code.toLowerCase().contains(_search) ||
          ...
          course.university.toLowerCase().contains(_search);
  ...
  List<String> get levels => ['All', '6', '7', '8'];
...
class _CAOSearchPageState extends State<CAOSearchPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<CourseProvider>(context, listen: false).loadUserInterestsAndCourses('assets/cao_list_25.csv');
    });
  }
  @override
  void dispose() {
          ...
          builder: (_, scrollController) {
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  Center(
                    child: Container(width: 40, height: 5, margin: const EdgeInsets.only(bottom: 10), decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(5))),
                  ),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text('Filter Options', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                      TextButton(
                        onPressed: () {
                  ...
                  Expanded(
                    child: ListView(
                      ...
                      children: [
                        _buildFilterDropdown("Job Field", courseProvider.currentJobFieldFilter ?? 'All', courseProvider.jobFields, (val) => courseProvider.setFilter(jobField: val), context),
                        _buildFilterDropdown("Location", courseProvider.currentLocationFilter ?? 'All', courseProvider.locations, (val) => courseProvider.setFilter(location: val), context),
                        _buildFilterDropdown("University", courseProvider.currentUniversityFilter ?? 'All', courseProvider.universities, (val) => courseProvider.setFilter(university: val), context),
                        _buildFilterDropdown("Level", courseProvider.currentLevelFilter ?? 'All', courseProvider.levels, (val) => courseProvider.setFilter(level: val), context),
                        const SizedBox(height: 20),
                        SizedBox(
                          width: double.infinity,
                          child: ElevatedButton(
                            onPressed: () => Navigator.pop(context),
                            style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10.0))),
  ...
  Widget build(BuildContext context) {
    final courseProvider = Provider.of<CourseProvider>(context);
    return Scaffold(
      appBar: AppBar(title: const Text('CAO Course Search'), backgroundColor: Theme.of(context).primaryColor),
      body: Padding(
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: IconButton(icon: const Icon(Icons.clear), onPressed: () { _searchController.clear(); courseProvider.setSearch(''); }),
                filled: true,
                fillColor: Colors.grey[100],
              ),
              onChanged: (value) => courseProvider.setSearch(value),
            ),
            ...
            Row(
              ...
              children: [
                ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10.0)), elevation: 2),
                ),
              ],
            ),
            const SizedBox(height: 16),
                          ...
                          return Card(
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('${course.code} - ${course.title}', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold, color: Theme.of(context).primaryColor)),
                                  const SizedBox(height: 4),
                                  Text('${course.university} • Level ${course.level}', style: Theme.of(context).textTheme.bodyMedium),
                                  Text('${course.location} • ${course.jobField}', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.grey[600])),
                                ],
                              ),
                            ),
                          );
                          ...
                          return Padding(
                            ...
                            child: Center(
                              child: ElevatedButton(
                                ...
                                style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 30, vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30.0)), elevation: 3),
  ...
  Widget _buildFilterDropdown(String label, String value, List<String> options, ValueChanged<String?> onChanged, BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Padding(padding: const EdgeInsets.only(bottom: 8.0), child: Text(label, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold))),
          Container(
            width: double.infinity,
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(10.0), border: Border.all(color: Colors.blue.shade100)),
