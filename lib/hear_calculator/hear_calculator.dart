...
class HearCalculatorPage extends StatefulWidget {
  const HearCalculatorPage({super.key});

  @override
  State<HearCalculatorPage> createState() => _HearCalculatorPageState();
}

class _HearCalculatorPageState extends State<HearCalculatorPage> {
  ...
  void checkEligibility() {
    ...
    if (!incomeEligible) {
      setState(() {
        result =
            '❌ Not eligible: Your income exceeds the HEAR income limit (€${incomeLimit.toStringAsFixed(0)}).';
      });
      return;
    }
    ...
    if (indicators >= 2) {
      ...
      setState(() {
        result =
            '❌ Not eligible: You must meet two other criteria in addition to the income limit.';
      });
    }
  }
